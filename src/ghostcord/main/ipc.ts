...
handleSync(IpcEvents.GET_GIT_HASH, () => Ghostcord_GIT_HASH);
...
// Handler for VencordNative.ghostcord.relaunch() — used by Restart buttons in plugin settings
handle(IpcEvents.RELAUNCH_APP, async () => {
    ...
    // On packaged Windows (NSIS/Squirrel), app.relaunch() can fail silently
    // because process.execPath points to the internal Electron binary, not the launcher.
    // Use spawn to restart the main executable directly.
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.GhostcordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { GhostcordDir } = State.store;
    if (!GhostcordDir) return;

    const stats = await stat(GhostcordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(GhostcordDir);
});
