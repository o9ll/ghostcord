...
// When Discord's OOP overlay crashes (always in Ghostcord — we're not discord.exe),
...
export function makeLinksOpenExternally(win: BrowserWindow) {
    win.webContents.setWindowOpenHandler(({ url, frameName, features }) => {
        ...
        if (isDiscordPopout || (frameName.startsWith("DISCORD_") && pathname === "/popout" && DISCORD_HOSTNAMES.includes(hostname))) {
            ...
            // The game overlay can never work in Ghostcord (wrong process name),
    ...
    win.webContents.on("did-create-window", (childWin, { frameName, options, url }: any) => {
        ...
        if (isPopout) {
            ...
            // Hide the window immediately to avoid white flash, then close
            // as soon as navigation to the real link triggers.
            childWin.hide();
            ...
            // Safety net: close anyway if nothing navigates within 2s
            setTimeout(() => {
                if (!childWin.isDestroyed()) childWin.close();
            }, 2000);
        }
