...
function detectSource(json: any): "equicord" | "vencord" | "ghostcord" | "unknown" {
    // Heuristics based on keys present in the JSON
    ...
    // Equicord-specific plugins
    if (pluginNames.some(p => ["EquicordHelper", "EquicordCSS"].includes(p))) return "equicord";
    // Ghostcord-specific plugins
    if (pluginNames.some(p => ["GhostcordHelper", "equicordHelper"].includes(p))) return "ghostcord";
    // Fallback: if the file contains common Vencord keys
    if (pluginNames.length > 0) return "vencord";

    return "unknown";
}

function cleanForGhostcord(json: any): any {
    ...
    // Remove Equicord/Vencord-only plugins that don't exist in Ghostcord
    ...
    async function processFile(file: File) {
        ...
        try {
            ...
            const sourceLabel =
                ...
                source === "ghostcord" ? "Ghostcord" : "inconnu";

            const cleaned = cleanForGhostcord(json);

            // Send to main process via IPC to write settings
            ...
            setStatus("success");
            setMessage(`✅ ${sourceLabel} settings imported successfully! Restart Ghostcord to apply.`);
        } catch (err: any) {
            setStatus("error");
    ...
    return (
        <div className={cl("category")}>
            ...
            <BaseText size="sm" style={{ marginBottom: "12px", opacity: 0.7 }}>
                Drag & drop your Equicord or Vencord backup JSON file here to import your settings into Ghostcord.
                Plugin-specific settings that don't exist in Ghostcord will be automatically removed.
            </BaseText>
