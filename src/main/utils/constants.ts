...
export const DATA_DIR = process.env.GHOSTCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "GhostcordData", suffix)
        : join(app.getPath("userData"), "..", "Ghostcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Ghostcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
