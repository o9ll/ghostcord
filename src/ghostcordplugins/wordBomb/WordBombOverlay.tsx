...
import { getGroqKey } from "../ghostcordAI/groqManager";

const ComponentDispatch = findByPropsLazy("dispatchToLastSubscribed");

const DICT_URLS = [
    "https://raw.githubusercontent.com/loayamin/arabic-words/master/word-list.txt",
    "https://raw.githubusercontent.com/a3f/arabic-wordlists/master/arabeyes.txt"
];
...
function mountOverlay() {
    ...
    try {
        if (createRoot) {
            ...
            console.error("[WordBomb] No React renderer available");
        }
    } catch (e) {
        console.error("[WordBomb] Mount error:", e);
    }
}
...
export function WordBombOverlay() {
    ...
    const sendWord = async (word: string) => {
        ...
        if (safeMode) {
            ...
            if (!groqKey) {
                setDefinition(t("Error: Groq API key missing in GhostcordAI."));
            } else {
                fetch("https://api.groq.com/openai/v1/chat/completions", {
                    ...
                    body: JSON.stringify({
                        ...
                        temperature: 0.7,
                        max_tokens: 150,
                        messages: [{ role: "user", content: `Give a very short definition (1 simple sentence) for the following word, explaining what it actually is, without giving its grammatical nature. Word: "${word}"` }],
                    }),
                })
                    .then(r => r.json())
